...
XLSX читается в браузере только в интерфейсе суперадминистратора через `read-excel-file`. До parsing проверяются размер до 5 МБ, допустимый MIME при его наличии, ZIP/XLSX signature и central directory. Encrypted entries, небезопасные archive paths, macros, external links, embedded/ActiveX objects, чрезмерное сжатие и распакованный объём отклоняются.

Допустимо не более 12 листов, 5 000 строк и 64 колонок на лист, 100 000 ячеек на лист и 250 000 ячеек суммарно. ZIP содержит не более 256 entries, не более 10 МБ на entry и не более 40 МБ распакованных данных. Формулы не вычисляются: parser читает только сохранённое значение ячейки. Библиотека не предоставляет hard timeout или потоковые per-row limits в browser API, поэтому остаётся ограниченный риск синхронной XML-обработки внутри уже ограниченного объёма.
