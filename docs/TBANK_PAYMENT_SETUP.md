...
Если владелец сервиса сделал возврат в кабинете T-Bank, Администратор сначала выбирает «Сверить с T-Bank». Backend вызывает `GetState`; статус `REFUNDED` означает полный возврат, а `PARTIAL_REFUNDED` — частичный. При полном возврате приложение не вызывает `/Cancel`, а атомарно создаёт `RefundTransaction`, списывает номинальную сумму с основного баланса и создаёт ledger/audit. Для `terminalMode=live` также создаётся запись возврата в реестре «Мой налог», для `test` — нет. Повторная сверка идемпотентна.

Частичные возвраты автоматически не учитываются: платёж и возврат переводятся на ручную проверку без списания баланса и без записи НПД. Если основного баланса недостаточно, списание в минус также не выполняется и требуется ручная проверка.

Операция «Зафиксировать возврат по банку» остаётся резервным сценарием для `live`-платежа, когда возврат подтверждён банковскими документами, но автоматическая сверка с T-Bank не обнаружила его.
