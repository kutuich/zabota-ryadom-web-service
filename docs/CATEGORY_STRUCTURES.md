...
Active нельзя редактировать. Импорт всегда создаёт новую draft-версию, активация архивирует прежнюю active того же scope и записывает `activatedAt`. Откат также не возвращает старую запись в active: он клонирует выбранную версию в новый draft, который нужно проверить и активировать отдельно. Сравнение версий показывает добавленные, удалённые и изменённые категории, задачи и ценовые правила.
