...
assert.deepEqual(adminFinanceItems, ["Балансы", "Платежи", "Мой налог", "Резерв визитов"]);
...
assert.match(requestCreationForm, /Кому нужна помощь/);
assert.match(requestCreationForm, /Состояние Подопечного/);
assert.match(requestCreationForm, /Что нужно сделать/);
assert.match(requestCreationForm, /Как часто и когда нужна помощь/);
assert.match(requestCreationForm, /Добавить ещё один визит/);
assert.match(requestCreationForm, /Создать заявку/);
assert.match(requestCreationForm, /quoteRequestId/);
assert.match(requestCreationForm, /request-quote__visits/);
...
assert.match(visitReservePanel, /Денежный резерв/);
assert.match(visitReservePanel, /Бонусные обязательства/);
assert.match(visitReservePanel, /Общий операционный риск/);
assert.match(visitReservePanel, /Автоматическая сверка/);
assert.match(visitReservePanel, /Последнее успешное выполнение/);
...
assert.match(chatPanel, /Согласованная стоимость помощи за график/);
assert.match(chatPanel, /Сервисный сбор Заказчика за график/);
assert.match(chatPanel, /Сервисный сбор Помощника за график/);
assert.match(chatPanel, /Стоимость каждого визита/);
assert.match(chatPanel, /agreedVisits/);
...
assert.match(chatPanel, /Оплата помощи производится напрямую между сторонами/);
assert.doesNotMatch(chatPanel, /previewAmount - 50|Доход помощника после сервисного сбора/);
...
assert.match(agreedTermsSummary, /Ориентир общих расходов Заказчика/);
assert.match(agreedTermsSummary, /Оплата помощи/);
assert.match(agreedTermsSummary, /Напрямую между сторонами/);
assert.doesNotMatch(agreedTermsSummary, /Помощник получит|Доход после сервисного сбора/);
...
assert.match(priceSummary, /Оплата помощи/);
assert.match(priceSummary, /Напрямую от Заказчика/);
assert.doesNotMatch(priceSummary, /Доход после сервисного сбора/);
...
assert.match(visualAuditShowcase, /data-workflow-version="2"/);
assert.match(visualAuditShowcase, /Лёгкая уборка/);
assert.match(visualAuditShowcase, /Мытьё посуды/);
assert.match(visualAuditShowcase, /Приготовить простую еду/);
assert.match(visualAuditShowcase, /Купить продукты/);
assert.match(visualAuditShowcase, /15 визитов/);
assert.match(visualAuditShowcase, /30 часов/);
assert.match(visualAuditShowcase, /Сверить визиты вручную/);
assert.match(visualAuditShowcase, /data-audit-field-error/);
...
const workflowVisualAuditScript = read("../../scripts/visual-audit.mjs");
assert.match(workflowVisualAuditScript, /\/app\/audit\/client/);
assert.match(workflowVisualAuditScript, /\/app\/audit\/performer/);
assert.match(workflowVisualAuditScript, /\/app\/audit\/admin/);
assert.match(workflowVisualAuditScript, /1440, height: 900/);
assert.match(workflowVisualAuditScript, /1280, height: 800/);
assert.match(workflowVisualAuditScript, /390, height: 844/);
assert.match(workflowVisualAuditScript, /360, height: 800/);
assert.match(workflowVisualAuditScript, /Horizontal overflow/);
assert.doesNotMatch(workflowVisualAuditScript, /fillClientRequestDraft|client@zabota\.local|performer@zabota\.local/);
...
assert.doesNotMatch(categoryStructuresPage, /Подготовить откат/);
assert.match(categoryStructuresPage, /Создать новую версию на основе этой/);
...
assert.match(categoryStructuresPage, /Показать архивные версии/);
assert.match(categoryStructuresPage, /Все, включая архивные/);
assert.match(categoryStructuresPage, /if \(tab === "Версии"\) setStatusFilter\("working"\)/);
...
assert.match(categoryStructuresPage, /Удалить черновик/);
assert.match(categoryStructuresPage, /Удалить окончательно/);
assert.match(categoryStructuresPage, /Проверить зависимости/);
assert.match(categoryStructuresPage, /Экстренно отключить структуру/);
assert.match(categoryStructuresPage, /Создать новую версию на основе этой/);
assert.match(categoryStructuresPage, /Старая версия не будет активирована повторно/);
assert.match(categoryStructuresPage, /Начать актуализацию/);
...
assert.match(categoryStructureStyles, /\.category-structure-danger-zone/);
...
assert.match(customerCategoryUi, /Заявка временно скрыта от Помощников/);
assert.match(customerCategoryUi, /Подтвердить обновлённые данные/);
...
assert.match(categoryApi, /\/dependencies/);
assert.match(categoryApi, /emergency-disable/);
assert.match(categoryApi, /request-updates/);
