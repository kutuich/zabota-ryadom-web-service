...
export function safetyApplies(applicability: any, values: any) { if (!applicability || Object.keys(applicability).length === 0) return true; const conditions = Array.isArray(applicability.conditions) ? applicability.conditions : []; return conditions.length === 0 || conditions.every((condition: any) => Object.values(values).some((nodeValues: any) => nodeValues?.[condition.fieldId] === condition.value)); }
